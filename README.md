...
6. Dán Client ID đã copy vào các file môi trường ở **cả 2 nơi** (phải đảm bảo giống hệt nhau):
   - Backend: Ghi nhận tại `server/.env` ở biến `GOOGLE_CLIENT_ID`
   - Frontend: Ghi nhận tại `client/.env` ở biến `VITE_GOOGLE_CLIENT_ID`
   > [!IMPORTANT]
...
Các file `.env` được khai báo trong `.gitignore` để đảm bảo an toàn bảo mật. Bạn cần tạo các file `.env` bằng cách nhân bản từ `.env.example` ở mỗi thư mục và điền giá trị thật tương ứng.

### 1. Cấu hình Backend (`server/.env`)

| Tên biến | Ý nghĩa | Cách lấy giá trị / Ví dụ |
| :--- | :--- | :--- |
| `PORT` | Cổng cổng dịch vụ Backend lắng nghe nhận yêu cầu. | Mặc định: `4000` |
...
### 2. Cấu hình Frontend (`client/.env`)

| Tên biến | Ý nghĩa | Cách lấy giá trị / Ví dụ |
| :--- | :--- | :--- |
| `VITE_API_BASE_URL` | Đường dẫn API Endpoint gốc mà Client sẽ gọi. | Dev: `http://localhost:4000/api`<br>Prod: `https://research4student.io.vn/api` |
| `VITE_GOOGLE_CLIENT_ID` | Client ID đồng bộ để render nút đăng nhập Google ở Client. | **Phải khớp 100%** với `GOOGLE_CLIENT_ID` cấu hình ở server. |
...
pm2 startup
```
*Ghi chú*: Hãy đảm bảo file `server/.env` ở trên VPS đã được cấu hình biến `CLIENT_URL=https://research4student.io.vn` để chính sách CORS của Node.js chấp nhận yêu cầu từ tên miền chính thức của bạn.
...
|---|---|---|
| Đăng nhập Google | `server/.env`: `GOOGLE_CLIENT_ID` + `client/.env`: `VITE_GOOGLE_CLIENT_ID` (2 giá trị phải GIỐNG NHAU) | Build lại client (`npm run build`) và restart server (`pm2 restart r4s-api`) |
| CAPTCHA (Cloudflare Turnstile) | `server/.env`: `TURNSTILE_SECRET_KEY` + `client/.env`: `VITE_TURNSTILE_SITE_KEY` | Build lại client và restart server |
| Lưu file qua Discord | `server/.env`: `DISCORD_BOT_TOKEN` + `DISCORD_STORAGE_CHANNEL_ID` | Restart server |
